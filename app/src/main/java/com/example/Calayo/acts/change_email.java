                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            if (!task.getResult().isEmpty()) {
                                ...
                                db.collection("users").document(temp.getLoggedin()).update("email", email)
                            ...
                            if (e instanceof FirebaseFirestoreException) {
                                ...
                                if (firestoreException.getCode() == FirebaseFirestoreException.Code.UNAVAILABLE) {
                                    Toast.makeText(change_email.this, "No internet connection", Toast.LENGTH_LONG).show();
                                    logEvent("FIRESTORE_UNAVAILABLE", "No internet connection", null, "userRegisterAct");
                                    return;
                                }
                            }
                            Log.e("Firestore", "Error getting document", e);
                            Toast.makeText(change_email.this, "Error checking email. Please try again.", Toast.LENGTH_SHORT).show();

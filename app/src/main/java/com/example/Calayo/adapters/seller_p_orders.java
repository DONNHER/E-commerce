...
public class seller_p_orders  extends RecyclerView.Adapter<seller_p_orders.PageViewHolder> {
    private List<Order> items;
    FragmentActivity fragmentAct;
    private tempStorage temp = tempStorage.getInstance();
    private FirebaseFirestore db = FirebaseFirestore.getInstance();

    public seller_p_orders(List<Order> items, FragmentActivity fragmentActivity) {
        this.items = items;
    ...
    public void onBindViewHolder(@NonNull PageViewHolder holder, int position) {
        ...
        holder.approve.setOnClickListener(v -> {
            ...
            temp.getSelectedOrder().setStatus("Approved");
        });
